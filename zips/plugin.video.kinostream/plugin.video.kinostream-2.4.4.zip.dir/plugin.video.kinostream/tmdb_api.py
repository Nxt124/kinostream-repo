...
def search_movies(api_key, query, year='', page=1):
    params = {'query': query, 'page': page}
    if year:
        params['year'] = str(year)
    data = _get('/search/movie', params, api_key)
    return [_parse_movie(r) for r in data.get('results', [])], data.get('total_pages', 1)


def search_tvshows(api_key, query, page=1):
    data = _get('/search/tv', {'query': query, 'page': page}, api_key)
    return [_parse_tv(r) for r in data.get('results', [])], data.get('total_pages', 1)
