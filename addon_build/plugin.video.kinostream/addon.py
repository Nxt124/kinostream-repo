...
def main_menu():
    ...
    if favorites:
        _add_dir('[B]Oblibene[/B]', _url(mode='favorites_root'),
...
def media_root(media_type):
    ...
    recent = _load_recent_played()
    if any(v.get('media_type') == media_type for v in recent.values()):
        _add_dir('Naposledy prehrane',
                 _url(mode='recent_list', media_type=media_type),
                 img='DefaultRecentlyAddedMovies.png')
    ...
    favorites = _load_favorites()
    if any(v.get('media_type') == media_type for v in favorites.values()):
        _add_dir('Oblibene',
                 _url(mode='favorites_list', media_type=media_type),
                 img='DefaultFavourites.png')
...
def _add_tmdb_item(item, media_type):
    ...
    if _favorite_exists(media_type, tmdb_id, title):
        li.addContextMenuItems([
            ('Odebrat z oblibenych', 'RunPlugin(%s)' % _url(mode='favorite_remove', **favorite_params))
        ])
    else:
        li.addContextMenuItems([
            ('Pridat do oblibenych', 'RunPlugin(%s)' % _url(mode='favorite_add', **favorite_params))
        ])
...
def show_episode_variants(variants_json, season, ep_num, ep_name='', ep_plot=''):
    ...
    for v in variants:
        ...
        xbmcplugin.addDirectoryItem(addon_handle,
            _url(mode='play', ident=ident, title=name, size=size, media_type='tvshow'), li, False)
...
def show_favorites_list(media_type='all'):
    ...
    if not favorites:
        _notify('Zadne oblibene polozky')
        return _fail()
    ...
    for fav_key, data in sorted(favorites.items(), key=lambda i: i[1].get('time', 0), reverse=True):
        ...
        li.addContextMenuItems([
            ('Odebrat z oblibenych', 'RunPlugin(%s)' % _url(mode='favorite_remove', media_type=data.get('media_type', 'movie'), tmdb_id=data.get('tmdb_id', ''), title=data.get('title', '')))
        ])
...
def favorites_clear():
    ...
    if not favorites:
        _notify('Oblibene jsou prazdne')
        return _fail()
    if xbmcgui.Dialog().yesno('Kino Stream', 'Opravdu chcete vymazat vsechny oblibene?'):
        _save_favorites({})
        _notify('Oblibene byly vymazany')
        xbmc.executebuiltin('Container.Refresh')
    return _fail()
...
def play_file(ident, title='', size='', img='', media_type='movie'):
